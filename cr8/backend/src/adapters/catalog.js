...
export function createCatalogAdapter() {
  return {
    async searchTracks({ title, artist, album, offset = 0 }) {
    ...
    async searchAlbums({ title, artist, offset = 0 }) {
    ...
    async searchArtists({ name, offset = 0 }) {
    ...
    async browseArtistAlbums(artistMbid, { offset = 0 } = {}) {
    ...
    async lookup(mbid, type = 'recording') {
      let url
      if (type === 'recording') {
